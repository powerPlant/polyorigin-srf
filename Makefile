...
SYMLINK_COMMAND=ln -sf $(IMAGE_FILE) julia
...
# add more fields to retain any rogue - separators i.e. cut -d- -f2,3,4,5
$(LOCAL_IMAGE_DEPS): LOCAL_RECIPE_FILE=$(shell grep "^From: $@/" $(RECIPE_FILE_TARGET) | cut -d' ' -f2 | xargs -I{} basename {} .sif | cut -d- -f2)
$(LOCAL_IMAGE_DEPS):
